...
//header('Content-type: application/json');
        ...
        $target = $data['target'];
//        echo strtotime($data['departure_date']);
//        echo "</br>";
//        echo strtotime($data['return_date']);
//        echo "</br>";
        
//         preg_replace('/-/', ' ', $airport_from_cod);
        $airport_from_name = explode('-', $airport_from_cod);
        $airport_to_name = explode('-', $airport_to_cod);
        $airport_from = $this->cms_model->get_data_by_like_row('name', $airport_from_name[0], 'airports');
        $airport_to = $this->cms_model->get_data_by_like_row('name', $airport_to_name[0], 'airports');
        ...
        $jsonData = array(
            ...
            "airport_from_code" => $airport_from_code,
            "airport_to_code" => $airport_to_code,
            "departure_date" => $departure_date,
            "return_date" => $return_date,
            "adult_flight" => $adult_flight,
            "child_flight" => $child_flight,
            "infant_flight" => $infant_flight,
...
//        $data['flight_details'] = json_decode($result2, true);
        ...
        if($data['journey_type'] == 'OneWay'){
            $page = "one_way";
        } else if($data['journey_type'] == 'Circle') {
          $page = "two_way";
        }
        
        $this->load->view('font_end/includes/header', $this->data);
        $this->load->view('font_end/'.$page, $data);
        $this->load->view('font_end/includes/footer');
    }
...
//        die;
    ...
    public function privacy_policy() {
        $data['cms'] = $this->cms_model->view_data('5', 'cms_pages');


        $this->load->view('font_end/includes/header', $this->data);
        $this->load->view('font_end/privacy_policy', $data);
        $this->load->view('font_end/includes/footer');
    }
    
    public function terms_and_conditions() {
        $data['cms'] = $this->cms_model->view_data('6', 'cms_pages');


        $this->load->view('font_end/includes/header', $this->data);
        $this->load->view('font_end/terms_and_conditions', $data);
        $this->load->view('font_end/includes/footer');
    }
...
//        $data['header_banners'] = $this->cms_model->view_data('12', 'header_banners');
    ...
    public function user_register() {
        ...
        $row = $this->cms_model->get_data_by_type_or('email', $email, 'mobile', $mobile, 'users');
//        print_r($row);
//        die;
        if ($row == '') {
            $chars2 = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
            $substr1 = substr(str_shuffle($chars2), 0, 3);
            $data['user_id'] = $substr1 . strtotime(date('d-m-Y h:i:s'));
            $data['date_of_reg'] = strtotime(date('d-m-Y'));
...
//        die; 
            ...
            $substr = substr(str_shuffle($chars), 0, 5);
            $session_id = $substr . strtotime(date('d-M-Y h:i A'));

//            $otp = rand(1000, 9999);
//            $data['otp'] = $otp;

            $insert = $this->cms_model->insert_data('users', $data);
       ...
       Thank you for registering with us";
                ...
                $mail = mail($email, $site, $message, $headers);
//                $send = $this->email->send();

//                if ($mail) {

                    $sess_arr = array(
                        'sess_id' => $data['user_id'],
